...
const getAllBlogs = async (req, res) => {
    try {
        ...
        console.log("BlogsWithoutLikesAndDislikes");
        console.log(blogs);

        const blogsWithLikes = await Promise.all(
            blogs.map(async (blog) => {
                const stats = await getLikeStats(blog.id);
        ...
        console.log("BlogsWithLikesPresentsHere");
        console.log(blogsWithLikes);

        res.status(200).json({
            message: "Blogs fetched successfully",
            count: blogsWithLikes.length,
...
const updateBlog = async (req, res) => {
    try {
        ...
        const image_url = req.file ? `/uploads/blogs/${req.file.filename}` : blog.image_url;
        await blog.update({
            title,
            content,
            category: category || "General",
